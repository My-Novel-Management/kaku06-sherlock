...
def ch_main(w: World):
    return w.chapter('main',
            ...
            w.plot_note("１話（安楽椅子）：$cradesが$heroを見つけて、魔王退治を依頼する。現場に行かずに事件解決（しかし情報間違っていたので後で推理し直す）"),
            w.plot_note("２話（アリバイ・人物偽装）：ボヘミア醜聞で$ail登場"),
            w.plot_note("３話（事実誤認）：$pan仲間回"),
            w.plot_note("４話（密室）："),
            w.plot_note("５話（）：$emil仲間回"),
            w.plot_note("６話（クローズド）：犬の仕業みたいな謎の殺人事件発生"),
            w.plot_note("７話（奇妙な凶器）：魔獣の討伐依頼"),
            w.plot_note("８話（心理トリック）：$heroが滝に落ちる"),
            w.plot_note("９話（）：実は謎の男の正体は$heroだった"),
            w.plot_note("１０話（叙述トリック）：戻ってきた$heroが"),
